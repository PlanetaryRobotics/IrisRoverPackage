...
namespace CubeRover
{
            ...
            const NATIVE_INT_TYPE instance)
    {
        ...
        while (!success)
        {
            ...
            while (no_transition_count < MAX_FSM_NO_TRANSITION_COUNT)
            {
                ...
                if (m_current_state == CubeRoverNetworkManager::UDP_CONNECTED)
                { // Connection & transport layer established
                    success = true;
                    break;
                }
            }
            ...
            Fw::Buffer &fwBuffer)
    {
        ...
        Wf121::ErrorCode e = m_crnm.SendUdpData(buffer, payloadSize, 100000); // TODO: FIXME: What is an appropriate timeout 1s check units
        update();
    }
    ...
    void NetworkManagerComponentImpl::update()
    {
        ...
        {
            // Every `telem_send_limit_cnt_max` `update` calls, downlink:
            telem_send_limit_cnt = 0;
    ...
    void NetworkManagerComponentImpl::getUplinkDatagram()
    {
        ...
        if (bytesRead == sizeof(Wf121::DataSize16))
        {
            ...
            {
            } // FIXME: Error, bytes read didnt match payload size
        }
