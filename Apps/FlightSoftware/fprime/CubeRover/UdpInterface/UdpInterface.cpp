...
namespace CubeRover
{
  ...
  UdpInterfaceComponentImpl ::
      ...
      UdpInterfaceComponentImpl(
          const char *const compName) : UdpInterfaceComponentBase(compName)
#else
      UdpInterfaceComponentImpl(void)
#endif
                                        //,m_fd((uint32_t)(SCI_REG))
                                        ,
                                        m_packetsSent(0), m_bytesSent(0), m_packetsReceived(0), m_bytesReceived(0), m_packetsDropped(0), m_decodeErrors(0), m_firstSeq(true), m_currSeq(0)
  {
  }
          ...
          Fw::Buffer &fwBuffer)
  {
    ...
    if (fwBuffer.getsize() < sizeof(struct UdpHeader))
    { // Expects a datagram
