...
namespace Wf121
{
    namespace DirectMessage // Wf121::DirectMessage
    {
        ...
        // Current UDP Comms Interlock State (from an "ilock" message)
        // See `ground_reports.bgs` in the Radio FW for *a lot* more detail on
        // how this works (and the insane reasons why we have to hijack
        // innocuous BGAPI commands to make it work).
        enum class RadioUdpInterlockStatus
        {
            // Hercules doesn't have the interlock:
            // (Note: we effective start in this state - we have to request the interlock if we want int(x);):
            RADIO_HAS_INTERLOCK = 0x00,
            // Hercules has the interlock:
            HERC_HAS_INTERLOCK = 0x01,
        ...
        enum class RadioSwFatalError
        {
            ...
            CRITICAL_RESET = 0x90,
            // Radio's `anomaly_handling` caught a software exception and is going
            // to trigger a system reset:
            CRITICAL_RESET_SWE = 0x91
        };
        ...
        const uint8_t INTERLOCK_HEADER[] = /**/ "ilock:";
        ...
        public:
             ...
             * @return Returns the number of bytes used (0 if no valid body was found).
             */
            uint8_t handleInterlockMessage(uint8_t body_len, uint8_t *body_data);
            /**
             ...
             */
            virtual void cb_dm_InterlockUpdate(const RadioUdpInterlockStatus status)
            {
                /* do nothing by default */
            }
        ...
        protected:
             ...
             * If there's a match, `cb_dm_InterlockUpdate` with `status` its argument.
             *
             * @param status Interlock status to send in the callback if there's a match.
             * @param ilock_buf Interlock buffer to check for.
             * @param body_len Length of buffer to check.
             * @param body_data Buffer to check.
             * @return uint8_t Number of bytes used (0 if no match).
             */
            uint8_t checkIfInterlockMatches(RadioUdpInterlockStatus status, const uint8_t *ilock_buf, uint8_t body_len, uint8_t *body_data);
        };
    }
}
