...
namespace Os {
  ...
  FRAM_File::Status FRAM_File::read(void * buffer, U32 start_time, U32 end_time) {
    ...
    while(current_data_tracker.data_name < start_time)
    {
        ...
        current_table_tracker.all += FRAM_TABLE_ENTRY_SIZE;
        if(current_table_tracker.all > FRAM_TABLE_PTR_END)
    ...
    while(current_data_tracker.data_name <= end_time)
    {
      ...
      if(current_table_tracker.all == table_end_ptr.all)
        return stat;
 ...
 */
  FRAM_File::Status FRAM_File::write(const void * buffer, uint8_t buff_size, uint32_t buff_name) {
    ...
    if(first_write)
    {
      ...
      table_start_data.data_location.all = FRAM_DATA_PTR_START;
    ...
    else
    {
      ...
      uint32_t projected_data_loc = table_end_data.data_location.all + table_end_data.data_size + buff_size;
      ...
      if(projected_data_loc > FRAM_DATA_PTR_END)
      {
        ...
        if(table_start_data.data_location.all < table_end_data.data_location.all /*check to see if end clear*/ 
            && table_start_data.data_location.all > start_projected_data_size /*check to see if start of data is clear*/)
        {
          ...
          table_end_ptr.all += FRAM_TABLE_ENTRY_SIZE;
        ...
        else
        {
          ...
          if(table_start_data.data_location.all > table_end_data.data_location.all)
            data_bytes_needed = buff_size - (table_start_data.data_location.all - (table_end_data.data_location.all + table_end_data.data_size));
          // We have space at the end but don't have enough space in the begining of the data section so we must go to the next start_table entry
            ...
            data_bytes_needed = start_projected_data_size - table_start_data.data_location.all;
          ...
          while(data_bytes_needed > 0)
          {
            ...
            table_start_ptr.all += FRAM_TABLE_ENTRY_SIZE;
      ...
      else
      {
        ...
        if(table_end_data.data_location.all < table_start_data.data_location.all && projected_data_loc > table_start_data.data_location.all)
        {
          // Calculate how many bytes will be needed to be given up
          int32_t data_bytes_needed = projected_data_loc - table_start_data.data_location.all;
          // Free next start locations until we have enough data bytes open
          while(data_bytes_needed > 0)
          {
            // Reduce the number of bytes needed by the size of the start 
  ...
  FRAM_File::Status FRAM_File::check_start_table_overrun()
  {
      ...
      {
        // If we're still using the first table address, we need to increment it
        if(table_end_ptr.all == FRAM_TABLE_PTR_START)
        {
