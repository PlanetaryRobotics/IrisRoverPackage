...
# What this does: It takes in a list of commands as a Python list with strings that follow those in the Commands and Telemetry spreadsheet
# in the google drive and returns a bitstring that reflects those commands

# How it does this: It works like a giant switch statement that reads one string element from the list at a time and adds the bits that that corresponds to
# in the variable "bitString" and converts numbers into their binary representation.

# Inputs: a Python list of commands as strings
# Outputs: a bitString corresponding to the commands given
			...
			if commands[i+1] == "Forward":
				...
				if commands[i+2] == "Distance":
					if len(commands) <= i+3: 
						raise Exception("not enough commands")
					bitString = bitString + "00"
					bitString = bitString + str(bin(int(commands[i+3])))[2:] # Takes in a number which is now a string->int->binary->string->chops off "0b" from the front
...
encode(["Driving", "Forward", "Distance", "5", "ESTOP","Driving", "Forward", "Distance", "5", "ESTOP" ])