  ...
  while (getline(_file, line)) {
    ...
    if (!end_header) {
      ...
      if (tmp_str == "element") {
        getline(liness, tmp_str, _separator);
        getline(liness, n);
        if (tmp_str == "vertex") num_vertex = stoi(n);
        if (tmp_str == "face") num_triangles = stoi(n);
      }
      ...
      if (!end_vertex && count < num_vertex) {
        ...
        cv::Point3f tmp_p;
        tmp_p.x = (float)stoi(x);
        tmp_p.y = (float)stoi(y);
        tmp_p.z = (float)stoi(z);
        ...
        tmp_triangle[0] = stoi(id0);
        tmp_triangle[1] = stoi(id1);
        tmp_triangle[2] = stoi(id2);
