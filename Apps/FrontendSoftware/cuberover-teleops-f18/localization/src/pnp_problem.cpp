...
// Constructor
PnPProblem::PnPProblem(std::vector<double> params) {
...
bool PnPProblem::estimatePose(const std::vector<cv::Point3f> &list_points3d,
    const std::vector<cv::Point2f> &list_points2d, int flags) {
  ...
  // Pose estimation
  bool correspondence = cv::solvePnP(list_points3d, list_points2d, A_matrix_, distCoeffs, rvec,
...
bool PnPProblem::backproject2DPoint(const Mesh *mesh, const cv::Point2f &point2d,
    cv::Point3f &point3d) {
