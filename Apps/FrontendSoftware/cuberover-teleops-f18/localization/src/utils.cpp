...
void drawArrow(cv::Mat image, cv::Point2i p, cv::Point2i q, cv::Scalar color, int arrowMagnitude,
...
// Prints elemnt in its own column so matrices are evenly spaced
void print_element(double value) {
  std::string output = ' ' + std::to_string(value);
  if(value < 0) output.erase(0, 1);
...
const std::vector<double> loadCameraParams(const std::string &path) {
  cv::FileStorage storage(path, cv::FileStorage::READ);
  double f, sx, sy, width, height;
  storage["focal_length"] >> f;
  storage["sx"] >> sx;
  storage["sy"] >> sy;
  storage["width"] >> width;
  storage["height"] >> height;

  const std::vector<double> params_cam = {
      width*f/sx,   // fx
      height*f/sy,  // fy
      width/2,      // cx
      height/2 };   // cy
...
void createFeatures(const std::string &featureName, int numKeypoints,
    cv::Ptr<cv::Feature2D> &detector, cv::Ptr<cv::Feature2D> &descriptor) {
  if (featureName == "ORB") {
...
cv::Ptr<cv::DescriptorMatcher> createMatcher(const std::string &featureName, bool useFLANN) {
  if (featureName == "ORB" || featureName == "BRISK" ||
      featureName == "AKAZE" || featureName == "BINBOOST") {
      if (useFLANN) {
