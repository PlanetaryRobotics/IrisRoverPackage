...
<template>
  <div id="functionalAdjustments">
    <!-- SLIDER ELEMENTS -->
    <div class="functionalSlider" v-for="(key, index) in adjustmentKeys" :key="index">
      <!-- HEADER -->
      <div class="functionalSlider__header">
        <p class="functionalSlider__header--title text__small">{{ key }}</p>
        <p class="functionalSlider__header--value text__small">{{ editData.adjustments[key] }}</p>
      </div>
      <!-- INPUT -->
      <input type="range" min="-1" max="1" v-model="editData.adjustments[key]" class="slider" step=".02"/>
    </div>
