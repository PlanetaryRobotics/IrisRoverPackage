<template>
  <div class="addTag modal">
    <h3 class="addTag__header text__main--bold modal__header">Add New Tag</h3>
    <input type="text" class="input__text--reg text__main" placeholder="New Tag" @keydown.space.prevent="spacePressed" v-model="tagName">
    <p class="text__small addTag__errorMessage">{{ errorMessage }}</p>
    <div class="addTag__buttons">
      <button class="button button__subtle addTag__buttons--input" @click="cancelClicked">CANCEL</button>
      <button class="button button__primary addTag__buttons--input" @click="saveClicked">SAVE</button>
    </div>
  </div>
</template>
