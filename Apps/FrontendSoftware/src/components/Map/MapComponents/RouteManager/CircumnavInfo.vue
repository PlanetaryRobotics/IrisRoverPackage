<template>
  <div>
    ...
    <div class="circumnav">
      ...
      <!-- EDIT ICON -->
      <svg @click.stop="openEditModal()" class="icon pencil" width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- TRASH ICON -->
      <svg @click.stop="openDeleteModal()" class="icon trash" width="15" height="19" viewBox="0 0 15 19" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
  ...
  methods: {
    ...
    openEditModal() {
      GridEventBus.$emit("OPEN_SEGMENT_MODAL", {route: this.route, segment: this.circumnavigation, segmentIndex: this.index, action: "EDIT"});
    },
    openDeleteModal() {
      let html = `Are you sure you want to delete circumnavigation `;
      html += `<span class='text__main--bold' style='color:red'>${this.circumnavigation.POICard.getName()}</span>? This action cannot be undone.`;
    ...
    deleteCallback() {
      this.deleteWaypoint({route: this.route, segment: this.circumnavigation});
    },
    cancelCallback() {
