<template>
  <div
    :class="isSmallModal() ? 'sidemodal__importance' : 'sidemodal'"
  ...
  >
    ...
    <div
      ...
      class="content__importance"
    >
      <div
        v-for="(level, index) of data"
        :key="index"
        :class="isCurrentImportance(level) ? 'importance--selected' : 'importance'"
        @click="setImportanceLevel(level)"
      >
        {{ level }}
      </div>
    </div>
    ...
    >
      ...
      <div
        v-for="(history, index) of data.slice(1, data.length)"
        :key="index"
