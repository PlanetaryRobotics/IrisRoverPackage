<template>
  <div style="width: 100%; height: 250px; margin: 0; border: 0; padding: 0;">
    <button @click="rotateX()">rotate X</button>
    <button @click="rotateY()">rotate Y</button>
    <canvas v-if="supportWebGL" id="rovermodel" ref="canvas" style="width: 100%; height: 230px;"></canvas>
    <div v-else>
      <slot>
        Your browser does not support <a href="http://khronos.org/webgl/wiki/Getting_a_WebGL_Implementation" style="color:#000">WebGL</a>.<br/>'
      </slot>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    hasListener() {
      // judge if there is mouse watching
  ...
  methods: {
    rotateX(){
      ...
      this.x += 0.1;
      rotation.x += 0.1;
      object.rotation.set(this.x, rotation.y, rotation.z);

      const c =document.getElementById("xz");
      const tdRover= c.getContext("2d");
      // clear canvas
      tdRover.clearRect(0, 0, 140, 140);

      // draw rect
      tdRover.translate(30,80);
      tdRover.rotate(0.1);
      tdRover.translate(-30,-80);

      tdRover.beginPath();
      this.stroke(tdRover, 30,50,30,80); // left kk
      this.stroke(tdRover, 90,50,30,50); // top kk
      this.stroke(tdRover, 90,80,90,50); // right kk
      this.stroke(tdRover, 30,80,90,80); // bottom
    ...
    rotateY(){
      ...
      object.rotation.set(rotation.x, this.y, rotation.z);

      const c =document.getElementById("yz");
      const tdRover= c.getContext("2d");
      // clear canvas
      tdRover.clearRect(0, 0, 140, 140);
