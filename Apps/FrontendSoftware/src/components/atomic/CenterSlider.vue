<template>
  <div id="centerSlider" class="container">
      <!-- HEADER -->
      <div class="functionalSlider__header">
        <p class="functionalSlider__header--title text__small headerText">{{ title }}</p>
        <p class="functionalSlider__header--value text__small headerText">
          {{ formatValue(childValue) }}
        </p>
      </div>
      <div class="center-range" :id="id" @mouseover="onMouseOver" @mouseleave="onMouseLeave">
        <div class="center-range__bar"></div>
        <input type="range" class="center-range__range"/>
        <!-- USER INPUT -->       
        <input type="range" 
               class="center-range__thumb" 
               :min="getNegativeRange()" 
               :max="range" 
               :step="step ? step : 0.2"
               v-model="childValue"
               @change="onChange"/>
  ...
  mounted() {
    ...
    window.addEventListener('load', () => {
        ...
        var RangeSlider = function(el) {
            ...
            // User's thumb
            this.range1 = elementByClassName(this.parent, 'center-range__thumb', 0);
            // Static input
            this.range2 = elementByClassName(this.parent, 'center-range__thumb', 1);
            this.bar = elementByClassName(this.parent, 'center-range__bar', 0);
            this.width = this.range1.offsetWidth;
            this.min = 0;
            this.max = parseFloat(that.range) * 2;
            //this.displayElement = elementByClassName(this.parent, 'double-range__value', 0);

            this.update = function() {
                var rangeValue1 = parseFloat(this.range1.value) + that.range;
                var rangeValue2 = parseFloat(this.range2.value) + that.range;
        ...
        var ranges = arrayByClassName(document, 'center-range')
            .reduce(function (prev, htmlNodes) {
                return prev.concat(arrayByClassName(htmlNodes, 'center-range__thumb'));
...
.center-range__range, .center-range__thumb {
  z-index: 1;
  pointer-events: none;
  position: absolute;
  overflow: hidden;
...
.center-range__range ::-webkit-slider-thumb{
...
.center-range__thumb::-webkit-slider-thumb {
  pointer-events: auto;
  position: relative;
  z-index: 1;
  outline: 0;
