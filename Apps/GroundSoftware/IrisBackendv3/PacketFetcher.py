...
def build_image_from_ICP(packet_bytes, image, image_index, block_number_list):
    ...
    CPH = packet_bytes[:5]
    seq_num, vlp_len, checksum = struct.unpack(endianness_code+' B H 2s', CPH)
    ...
    while len(VLP) > 0 and image_index < image.size:
        ...
        hashedId, totalBlocks, blockNumber, blockLen = struct.unpack(endianness_code+' H B B B', header)
        ...
        while len(VLP) > 0 and VLP[:4] != file_magic and image_index < image.size:
            image[image_index], VLP = VLP[0], VLP[1:] # Strip off first bit into image
...
def main():
    ...
    for idx, packet in enumerate(packets):
        ...
        printraw(content)
        image_index, block_number_list = build_image_from_ICP(content, image, image_index, block_number_list)
