"""
Starting implementation of an IPC Image File builder.
...
Also run a data source like a Transceiver to supply packets
...
app = ipc.IpcAppHelper("ImageBuilder")
app.setLogLevel('VERBOSE')

# Where output files get saved (w.r.t GroundSoftware)
OUTPUT_DIR_ROOT: Final[str] = './out/image_builder_out/'

# Prefix to apply to all files:
FILE_PREFIX_DEFAULT: Final[str] = 'IMG_IRIS'

# Default number of lines in an image:
DEFAULT_IMAGE_LINE_COUNT: Final[int] = 1944
# Full with of image file, with any extra line data included (blank memory
# regions).
# DLing all pages in a line in case of shift (extra space doesn't matter much
# when compressed)
DEFAULT_FULL_IMAGE_WIDTH: Final[int] = 512*6
# Width of actual image region, with any extra regions (blank memory regions)
# excluded:
DEFAULT_CORE_IMAGE_WIDTH: Final[int] = 2592
...
class Image:
    ...
    # downsampling, etc.):
    full_image_columns: int = DEFAULT_FULL_IMAGE_WIDTH
    core_image_columns: int = DEFAULT_CORE_IMAGE_WIDTH
    ...
    def file_name_base(self):
        ...
        """
        return (
            f"{self.FILE_PREFIX}__"
            f"Ulid{self.ulid_start}__"
            f"Fgid{self.file_group_id}__"
            f"Cam{self.camera_num}__"
            f"Cid{self.callback_id}"
        )
    ...
    def report(self) -> List[str]:
        ...
        return [
            ...
            f"Callback ID:\t {self.callback_id}",
            f"Camera Number:\t {self.camera_num}",
    ...
    def build_and_save_products(
        ...
        if make_quick_build:
            # Fastest data product.
            app.logger.info("\tMaking Quick Image . . .")
            start_sec = time.time()
        ...
        if make_median_build:
            # Typ. best quality data product.
            # Does this by building all interleaved frames as their own
            # image, interpolating unknown pixels, and then median-overlays
            # them to reduce noise and remove errant pixels present in only
            # one frame.
            # Also builds and caches all frames used by later steps.
            app.logger.info("\tMaking Median Image . . .")
            start_sec = time.time()
            image_median = image_data.build_median_frame()
            end_sec = time.time()
            path = os.path.join(
        ...
        if make_smart_build:
            # Uses all data from all interleaved frames in place (as if
            # from a single frame) to possibly produce a higher quality
            # image than median.
            app.logger.info("\tMaking Interpolated Smart-Build Image . . .")
            start_sec = time.time()
            image_smart = image_data.smart_build()
            end_sec = time.time()
            path = os.path.join(
                OUTPUT_DIR, self.file_name_base + ".interp.png")
            app.logger.notice(
                f"\t\t Done in {(end_sec-start_sec):.1f}s. "
                f"Saving to: {path} . . ."
            )
            pil = PIL.Image.fromarray(
                image_smart.astype('uint8'), 'L').rotate(180)
            pil.save(path)

        app.logger.notice(
            f"Done with all products for: {self.file_name_base}.")
    ...
    def decompress_and_extract_image_data_from_memory(self) -> img11.ImageDataRc11:
        """Decompresses image if necessary and uses RC11-style metadata
        embedded in the image to extract aligned image data."""
        start_sec = time.time()
        compressed_lines = self.assemble(equalize_lengths=False)
        # Decompress Flash Memory (if needed):
        decompressed_lines = [ic.decompress(
            cl) for cl in compressed_lines if len(cl) > 0]
        # Then rebuild the image from the (reconstructed) flash:
        image_data = img11.extract_image_from_flash_memory(
            b''.join(decompressed_lines), self.core_image_columns
        )
        end_sec = time.time()
        app.logger.notice(
            f'Extracted Image from Original Memory in {end_sec-start_sec:.01f}s'
        )
        return image_data
...
@dataclass
class LoopContext:
    """Context object for the main loop."""
    started: bool = False
    last_block_time_sec: float = 0
    # All FGIDs that have come in since the last export:
    fgids_since_last_export: Set[FileIdType] = field(
        default_factory=lambda: set()
    )


def payload_processing_update(
    opts,
    payloads: EnhancedPayloadCollection,
    decoder: BasicImageDecoder,
    context: LoopContext
) -> None:
    """Step to run every time a new payload comes in."""
    ...
    for block in payloads[FileBlockPayload]:
        ...
        context.fgids_since_last_export.add(block.file_group_id)

    # Check for end condition:
    downlink_complete = False

    # Check for a notice of a downlink completion:
    img_dl_end_events = [
        cast(EventPayload, e)
    ...
    if len(img_dl_end_events) > 0:
        app.logger.notice(
            f"Processing downlink b/c Downlink End Event: `{DOWNLINK_END_EVENT_NAME}`.")
        for img_dl_end in img_dl_end_events:
            app.logger.notice(
                f"\t Downlinking Finished: "
                f"FGID: {img_dl_end.args.get('file_group_id', 0)}, "
                f"CID: {img_dl_end.args.get('callback_id', 0)}"
            )

            if 'file_group_id' in img_dl_end.args:
                # Make sure this FGID is in the list of FGIDs to process:
                fgid_completed = img_dl_end.args['file_group_id']
                context.fgids_since_last_export.add(fgid_completed)
                # Warn if we don't have any data for this FGID:
                if fgid_completed not in decoder.images_in_progress:
                    app.logger.warning(
                        f"Received completion notice `{img_dl_end}` for image "
                        f"with FGID: `{fgid_completed}` but no matching FGID "
                        f"can be found in the image decoder."
                        f"Attempting to proceed with export of received "
                        f"blocks anyway."
                    )
    ...
    if downlink_complete:
        ...
        # If we didn't receive a notification, image DL probably didn't
        # complete and operators may want to request missing lines:
        for img_dl_end in img_dl_end_events:
...
def main(opts) -> None:
    ...
    while not done:
        try:
            ...
            payload_processing_update(opts, payloads, decoder, context)
        except KeyboardInterrupt as ki:
            # User can press key to export all in-progress images and quit.
            done = True
        except Exception as e:
            app.logger.error(
    ...
    app.logger.notice("Image Builder closing. Exporting everything . . .")
    decoder.export_and_process_all()
