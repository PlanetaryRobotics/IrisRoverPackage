...
load_cache()
# logs of stuff printed to console will be stored to `out/raw-console-logs`
setup_logger(str(settings['SAVE_FILE_PREFIX']))

# IF ERROR - update last number in FILE_PREFIX

serial_device = [cp.device for cp in list_ports.comports(
) if cp.serial_number is not None and serial_device_sn in cp.serial_number][0]
serial_device
