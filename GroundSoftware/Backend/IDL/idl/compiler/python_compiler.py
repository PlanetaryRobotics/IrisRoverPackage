    ...
    def run(cls,
        ...
        for input_file in input_dir_path.iterdir():
            ...
            if isinstance(file_type, idl_type.EnumType):
                ...
                # Create the parameter pairs used in the dictionary defined in the __str__ method.
                # (We don't want to treat the timestamp the same as the other unfixed fields)
                param_fields = copy.deepcopy(unfixed_fields)
                del param_fields["timestamp"]
                parameter_pairs = ",\n".join([python_template.PARAMETER_PAIR_TEMPLATE.format(name=field_name)
                                             for field_name in param_fields.keys()])
        ...
        msg_name_to_class_item_strings = []
        for msg_type_name, msg_type in all_message_types.items():
            ...
            msg_name_to_class_item_strings.append(python_template.NAME_TO_CLASS_MAP_ITEM_TEMPLATE.format(
                module_name=msg_module_name,
                class_name=msg_type_name))

        imports_string = "\n".join(msg_file_import_strings)
        dict_items_string = ",\n".join(msg_dict_item_strings)
        name_to_class_map_items_string = ",\n".join(msg_name_to_class_item_strings)
        utils_file_contents = python_template.MESSAGE_UTILS_FILE_TEMPLATE.format(
            message_file_imports=imports_string,
            name_to_class_items=name_to_class_map_items_string,
