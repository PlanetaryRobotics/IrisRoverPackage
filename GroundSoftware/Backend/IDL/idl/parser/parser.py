...
class Parser:
    ...
    def _parse_included_files(self, include_files_list: typing.List[str]) -> context.Context:
        ...
        for include_file_name in include_files_list:
            include_file_path = self.msg_folder_path.joinpath(include_file_name)

            if sys.version_info >= (3, 6):
                include_file_path = include_file_path.resolve(strict=True)
            else:
                include_file_path = include_file_path.resolve()
