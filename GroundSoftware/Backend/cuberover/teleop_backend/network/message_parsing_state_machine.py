...
class MessageParsingStateMachine:
    ...
    def new_bytes(self, byte_data: bytes):
        if self.__state == MessageParsingStateMachine.State.WAITING_FOR_HEADER:
            ...
            if len(self.__unknown_data_buffer) >= HEADER_SIZE_IN_BYTES:
                ...
                for i in range(self.__unknown_data_next_unchecked_start_index, possible_starts):
                    try:
                        ...
                        # If the data length of this message is zero (i.e. header only) then we can send off this
                        # message as competed and resume with the remaining data (if any).
                        if data_len == 0:
            ...
            print("Bytes remaining:", self.__data_bytes_remaining)
    ...
    def __handle_completed_message(self):
        if USING_IDL:
            msg = self.__msg_type.parse_from_serialized_bytes(self.__msg_buffer,
                                                              self.__input_bytes_will_be_big_endian)
        else:
